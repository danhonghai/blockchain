var GLOBAL_VAR = {
    ...
    button_down: false,
    init: false,
    url: "http://192.168.3.108:3104/kline?period=1min&symbol=ETH%2FBTC&from=1529288911&to=1529375311",
    depth: "http://192.168.3.108:3104/kline"
};
...
Chart.prototype.updateDataAndDisplay = function() {
    ...
    this.setTitle();
    ChartManager.getInstance().setCurrentDataSource("frame0.k0", "Bitcoin." + this._market_from + "." + this._time);
...
ChartManager.prototype.init = function() {
    ...
    DefaultTemplate.loadTemplate("frame0.k0", "Bitcoin");
...
Range.prototype.calcInterval = function() {
    ...
    var g = this.getRange();
    var b = 0;
    // change decimal kline gradations
...
MainDataSource.prototype.calcDecimalDigits = function(a) {
    ...
    var c = "" + a;
    if( (c.indexOf('E') != -1) || (c.indexOf('e') != -1) ) {
        c = eToString(c);
    }
    var b = c.indexOf(".");
    if (b < 0) {
...
LightTheme.prototype.__construct = function() {
    this._colors = [];
    this._colors[Theme.Color.Positive] = "#1f9c28";
    this._colors[Theme.Color.Negative] = "#de2017";
    ...
    this._colors[Theme.Color.Grid0] = "#bbb";
    this._colors[Theme.Color.Grid1] = "#afb1b3";
    this._colors[Theme.Color.Grid2] = "#ccc";
    this._colors[Theme.Color.Grid3] = "#bbb";
    this._colors[Theme.Color.Grid4] = "#aaa";
    this._colors[Theme.Color.TextPositive] = "#1f9c28";
    this._colors[Theme.Color.TextNegative] = "#de2017";
...
MainInfoPlotter.prototype.Draw = function(c) {
    ...
    if (D == "zh-cn") {
        ...
        if (!Plotter.drawString(c, "  量: " + C.volume.toFixed(4), a)) {
            return
        }
    } else {
        if (D == "en-us") {
            ...
            if (!Plotter.drawString(c, "  V: " + C.volume.toFixed(4), a)) {
                return
            }
        } else {
            if (D == "zh-tw") {
...
IndicatorInfoPlotter.prototype.Draw = function(c) {
    ...
    for (e = 0; e < d; e++) {
        ...
        var tofixed = p.getDataSource(this.getDataSourceName()).getDecimalDigits();
        if (i.getName() == 'VOLUME' || i.getName() == 'MA5' || i.getName() == 'MA7' || i.getName() == 'MA10' || i.getName() == 'MA30') {
            f = "  " + i.getName() + ": " + o.toFixed(4);
        } else {
            f = "  " + i.getName() + ": " + o.toFixed(tofixed);
        }
        g = i.getColor();
        if (g === undefined) {
...
RangePlotter.prototype.Draw = function(c) {
    ...
    for (var d in l) {
        ...
        // change decimal about gradations
        var tofixed = p.getDataSource(this.getDataSourceName()).getDecimalDigits();
        c.fillText(String.fromFloat(l[d], tofixed), a, k)
    }
    if (f.length > 0) {
...
LastVolumePlotter.prototype.Draw = function(b) {
    ...
    Plotter.drawLine(b, d, h, d + 3, h - 2);
    // change decimal right volume
    b.fillText(String.fromFloat(j, 4), d + 10, h)
}
;
...
function KLineMouseEvent() {
    $(document).ready(function() {
        ...
        // $('#chart_language_setting_div li a[name="zh-cn"]').trigger('click');
        if(getCookie('LANG') == 'cn') {
            chart_switch_language("zh-cn");
            // $('#chart_language_setting_div li a[name="zh-cn"]').trigger('click');
        } else {
            chart_switch_language("en-us");
            // $('#chart_language_setting_div li a[name="en-us"]').trigger('click');
        }
        // $('#chart_language_setting_div').hide();
...
var RequestData = function(showLoading) {
    ...
    if (!window.WebSocket) { //是否支持websocket
        $(document).ready(getklinedataAjax())
        return false;
    }
    getklinedataAjax();
    // getklinedataWebsocket();
};

var kline_one = 0;
var is_first_kline = {'1week': 0, '3day': 0, '1day': 0, '12hour': 0, '6hour': 0, '4hour': 0, '2hour': 0, '1hour': 0, '30min': 0, '15min': 0, '5min': 0, '3min': 0, '1min': 0};

function getklinedataWebsocket() {
    var b = kline.symbol + "_kline_" + GLOBAL_VAR.time_type;
    var type_arr = {
            '1week': '1w', '3day': '3d', '1day': '1d', '12hour': '12h', '6hour': '6h', '4hour': '4h', '2hour': '2h', '1hour': '1h', '30min': '30m', '15min': '15m', '5min': '5m', '3min': '3m', '1min': '1m'};
    if (kline_one == 1) {
        return;
    }
    kline_one = 1;

    var socketListen = socket.on('message' + kline.symbol, function(result) {
        if (is_first_kline[GLOBAL_VAR.time_type] == 0) {
            getklinedataAjaxOne();
        } else {
            // console.log(GLOBAL_VAR.time_type);
            json = JSON.parse(result.kline);
            var time_type = GLOBAL_VAR.time_type;
            var kline_data = GLOBAL_VAR.KLineData;
            var last_kline_number = kline_data.length - 1;
            var last_kline_array = kline_data[last_kline_number];
            if (undefined == eval(json.datas.data[type_arr[time_type]])) {
                return false;
            }

            // console.log(last_kline_array[0]);
            // console.log(eval(json.datas.data[type_arr[time_type]])[0]);

            if (last_kline_array[0] != eval(json.datas.data[type_arr[time_type]])[0]) {
                // console.log(time_type, '---');
                getklinedataAjaxOne();
                // kline_data[last_kline_number + 1] = eval(json.datas.data[type_arr[time_type]]);
            } else {
                kline_data[last_kline_number] = eval(json.datas.data[type_arr[time_type]]);
            }
            // kline_data = eval(json.datas.data[type_arr[GLOBAL_VAR.time_type]]);
            // console.log(kline_data);
            try {
                GLOBAL_VAR.KLineData = kline_data;
            ...
            // GLOBAL_VAR.TimeOutId = setTimeout(TwoSecondThread, 1000);
            $("#chart_loading").removeClass("activated");
            ChartManager.getInstance().redraw("All", false);

            var depth_result= JSON.parse(result.depth);
            if (depth_result.asks.length > 5 && depth_result.bids.length > 5) {
                kline.updateDepth(depth_result);
            }

            // sum & order
            sum = JSON.parse(result.sum);
            order = JSON.parse(result.order);
            parent.Trust.renderSum(sum);
            parent.Trust.renderOrder(order);
        }
    });
};

function getklinedataAjaxOne() {
    $.ajax({
        type: "GET",
        url: GLOBAL_VAR.url,
        dataType: "json",
        ...
        success: function(json) {
            ...
            is_first_kline[GLOBAL_VAR.time_type] = 1;
        }
    });
    /*$.get(GLOBAL_VAR.depth + '?depth=' + kline.symbol + '&v=' + Math.random(), function(result) {
        if (result.asks.length > 5 && result.bids.length > 5) {
            kline.updateDepth(result);
        }
    });*/
};
...
function refreshTemplate() {
    GLOBAL_VAR.chartMgr = DefaultTemplate.loadTemplate("frame0.k0", "Bitcoin");
    ChartManager.getInstance().redraw("All", true)
};
...
// 该方法暂时只适用于小于1的科学计数法,可根据需求整理成通用的方法
function eToString(tempValueStr) {
    if ((tempValueStr.indexOf('E') != -1) || (tempValueStr.indexOf('e') != -1))
    {
        var e = tempValueStr.indexOf('E') != -1 ? 'E' : 'e'; // 判断科学计数法中e是大写还是小写，用于下面字符串切分
        var arr = tempValueStr.split(e); // 字符串切分,如3.5e-7,切割成["3.5","-7"]
        var num = ""; // 临时变量
        var returnValue = "0."; // 此方法暂时只适用于小于的数据，因此可直接给结果字符串初始化赋值“0.”
        var power = 0; // 临时变量
        if (arr != null) // 判断arr是否为null
        {
            var tem = arr[0].split('.'); //将科学计数法数据前部分以“.”切分成两部分
            num = tem[0] + (tem[1] ? tem[1] : '0'); //拼接数据，若tem数组只有一个元素，则后面补0
            power = Math.abs(Number(arr[1])); //取到科学计数法e后面的数字,因为为负值，需要取绝对值，用于计算小数位数
            var length = power + 1 - num.length; //power+1为小数点后面位数，-num.length后，计算出需要补0的个数
            for(var i = 0; i < length; i++) { //for循环补0
                returnValue += '0';
            }
            returnValue += num; //补0后，加上后面非0字符串
        }
        return returnValue;
    }
};
